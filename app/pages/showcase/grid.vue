    ...
    <LayoutCenter variant="full">
      <div class="py-section space-y-component">
        ...
        <LayoutStack variant="centered">
          <LayoutHeading text="LayoutGrid Showcase" variant="hero" />
          <LayoutText text="Multi-column grid systems with responsive breakpoint behavior" variant="subtitle" />
        </LayoutStack>
...
&lt;/LayoutGrid&gt;</code></pre>
            ...
            <!-- Divider -->
            <LayoutDivider v-if="index < gridVariants.length - 1" variant="gradient" spacing="lg" />
          </div>
        </LayoutStack>

        <!-- Real-World Examples -->
        <div class="bg-elevated p-8 rounded-lg border border-muted">
          <LayoutHeading text="Real-World Grid Examples" variant="section" />
          
          <LayoutStack variant="spacious">
            <!-- Product Grid -->
            <div class="bg-circuit border-2 border-dashed border-emerald-500/30 p-6 rounded-lg">
              <h4 class="text-lg font-semibold mb-4 text-emerald-400">Product/Service Grid</h4>
              <LayoutGrid variant="cards">
                <UCard v-for="i in 3" :key="i" class="h-full">
                  ...
                  <LayoutStack variant="compact">
                    ...
                    <p class="text-sm text-muted">Brief product description with key benefits.</p>
                  </LayoutStack>
                  <template #footer>
                    <UButton label="Learn More" variant="outline" size="sm" block />
                  </template>
                </UCard>
              </LayoutGrid>
            </div>
            ...
            <div class="bg-circuit border-2 border-dashed border-blue-500/30 p-6 rounded-lg">
              <h4 class="text-lg font-semibold mb-4 text-blue-400">Team Members Grid</h4>
              <LayoutGrid variant="thirds">
                <div v-for="i in 3" :key="i" class="text-center">
                  <div class="w-24 h-24 mx-auto mb-3 bg-gradient-to-br from-secondary/20 to-primary/20 rounded-full flex items-center justify-center">
                    <UIcon name="i-lucide-user" class="size-8 text-secondary" />
                  </div>
                  <h4 class="font-semibold">Team Member {{ i }}</h4>
                  <p class="text-sm text-muted">Role & Expertise</p>
                </div>
              </LayoutGrid>
            </div>
          </LayoutStack>
