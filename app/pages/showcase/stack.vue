    ...
    <LayoutCenter variant="full">
      <div class="py-section space-y-component">
        ...
        <LayoutStack variant="centered">
          <LayoutHeading text="LayoutStack Showcase (FIXED)" variant="hero" />
          <LayoutText text="Visual exploration of all stack variants with distinct demo content" variant="subtitle" />
                ...
                <div class="flex items-start gap-2">
                  ...
                  <div>
                    ...
                    <ul class="mt-1 space-y-1 text-dimmed">
                      <li v-if="variant.name.includes('horizontal')">• Items flow horizontally instead of vertically</li>
                      <li v-if="variant.name === 'centered'">• Items center-align (different widths show this)</li>
                      <li v-if="variant.name === 'compact'">• Tighter gaps between items</li>
                      <li v-if="variant.name === 'spacious'">• Larger gaps between items</li>
                      <li v-if="variant.name.includes('between')">• Items spread to container edges</li>
                      <li v-if="variant.name.includes('responsive')">• Layout changes at md: breakpoint (768px)</li>
                    </ul>
                  </div>
                </div>
              </div>
...
&lt;/LayoutStack&gt;</code></pre>
            ...
            <!-- Divider -->
            <LayoutDivider v-if="index < stackVariants.length - 1" variant="gradient" spacing="lg" />
          </div>
        </LayoutStack>

        <!-- Responsive Testing Guide -->
        <div class="bg-elevated p-8 rounded-lg border border-muted">
          <LayoutHeading text="How to Test Responsive Variants" variant="section" />
          <div class="bg-blue-500/10 border border-blue-500/30 p-4 rounded-lg mt-4">
            <LayoutStack variant="compact">
              <p class="text-blue-400 font-medium">🖥️ Responsive Testing Guide:</p>
              ...
              <p class="text-xs text-dimmed mt-3">💡 <strong>Tip:</strong> Resize your browser window to see responsive changes in action!</p>
            </LayoutStack>
          </div>
        </div>
