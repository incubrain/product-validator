<!-- components/i/nav/Surrounds.vue -->
...
// Query surrounds for any collection
const { data: surrounds } = await useAsyncData(
  `${props.collection}-surrounds-${props.currentPath}`,
  () => queryCollectionItemSurroundings(props.collection, props.currentPath)
    .order(props.orderBy, props.orderDirection),
)

const previousItem = computed(() => surrounds.value?.[0])
const nextItem = computed(() => surrounds.value?.[1])
</script>
